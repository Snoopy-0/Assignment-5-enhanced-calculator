...
class Subtract(_Base):
    ...
    def execute(self, operands):
        if not operands:
            raise OperationError("Subtraction requires at least one operand")
        it = iter(operands)
...
class Multiply(_Base):
    ...
    def execute(self, operands):
        if not operands:
            raise OperationError("Multiplication requires at least one operand")
        result = 1.0
        for x in operands:
            result *= float(x)
        return result
...
class Divide(_Base):
    ...
    def execute(self, operands):
        if not operands:
            raise OperationError("Division requires at least one operand")
        it = iter(operands)
        result = float(next(it))
        for x in it:
            x = float(x)
            if x == 0.0:
                # EAFP style: try and handle error after detecting
...
class Power(_Base):
    ...
    def execute(self, operands):
        if len(operands) != 2:
            raise OperationError("Power requires exactly two operands")
...
class Root(_Base):
    ...
    def execute(self, operands):
        if len(operands) != 2:
            raise OperationError("Root requires exactly two operands: degree, value")
        degree, value = map(float, operands)
        if degree == 0:
            raise OperationError("Root degree cannot be zero")
        if value < 0 and degree % 2 == 0:
            raise OperationError("Even-degree root of negative number is not real")
